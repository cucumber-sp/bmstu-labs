...
class NumberSystemCalculator:
    ...
    def create_menu(self):
        ...
        action_menu.add_command(label="Перевести в 7-ую СС", command=self.convert_to_base7)
        action_menu.add_command(label="Перевести в 10-ую СС", command=self.convert_to_decimal)
    ...
    def create_widgets(self):
        ...
        ttk.Button(input_frame, text="⌫", command=self.backspace).pack(side="left", padx=2)
        ttk.Button(input_frame, text="C", command=self.clear_input).pack(side="left", padx=2)
        
        digits_frame = ttk.Frame(self.root, padding="10")
        digits_frame.pack(fill="x", padx=10)
        
        for i in range(7):
            btn = ttk.Button(digits_frame, text=str(i),
                           command=lambda x=i: self.add_digit(str(x)))
            btn.pack(side="left", padx=2)
        
        ttk.Button(digits_frame, text=".",
                  command=lambda: self.add_digit(".")).pack(side="left", padx=2)
        ttk.Button(digits_frame, text="-",
                  command=lambda: self.add_digit("-")).pack(side="left", padx=2)
        ...
        ttk.Button(conv_frame, text="→ 7-ая СС",
                  command=self.convert_to_base7).pack(side="left", padx=5)
        ttk.Button(conv_frame, text="→ 10-ая СС",
                  command=self.convert_to_decimal).pack(side="left", padx=5)
        ...
        ttk.Button(output_frame, text="C",
    ...
    def convert_to_decimal(self):
        ...
        if not validate_base7(number):
            messagebox.showerror("Ошибка",
                               "Введите корректное число в 7-ой системе счисления")
            return
